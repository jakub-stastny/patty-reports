...
(defn calculate-sales-growth-rate [prev-months _ {:keys [yoy-growth-rate]}]
  (let [year-index (int (/ (count prev-months) 12))]
...
(defn calculate-seasonal-adjustment-rate [prev-months month {:keys [yoy-growth-rate] :as inputs}]
  (nth (month-adjustment-ratios inputs) (dec (:month month))))

(defn calculate-pro-rata-factor [_ month {:keys [sales-start-date sales-end-date]}]
...
(defn process [prev-months month inputs results]
  (as-> results r
    (assoc r :sales-growth-rate (calculate-sales-growth-rate prev-months month inputs))
    (assoc r :seasonal-adjustment-rate (calculate-seasonal-adjustment-rate prev-months month inputs))
    (assoc r :pro-rata-factor (calculate-pro-rata-factor prev-months month inputs))))
